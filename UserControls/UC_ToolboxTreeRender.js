function UC_ToolboxTree($) {
	...
	this.Scripts = [];
		...
		this.Start = function() {
					...
					// Extended sample data with additional properties
			        const sampleData = 
			        {
			            "pages": 
			            [
			        ...
			        const graphData = {
			            "nodes": [
			                ...
			                "views": 7300,
			                "x": 152.57869477016035,
			                "y": 139.69278572792626,
			                "children": [
			                ],
			                "index": 2,
			                "vy": 0,
			                "vx": 0,
			                "fx": 152.57869477016035,
			                "fy": 139.69278572792626
			                },
			                {
			                "id": "4",
			                "title": "About Us",
			                ...
			                "views": 5200,
			                "x": 492.61037418345535,
			                "y": 496.46783876381375,
			                "children": [
			                ],
			                "index": 3,
			                "vy": 0,
			                "vx": 0,
			                "fx": 492.61037418345535,
			                "fy": 496.46783876381375
			                }
			            ],
			            "links": [
			            {
			            "source": {
			                ...
			                "vy": 0,
			                "vx": 0,
			                "fx": 160.3810881801417,
			                "fy": 241.49175563767668
			            },
			            "target": {
			                "id": "2",
			                "title": "Products",
			                ...
			                "vy": 0,
			                "vx": 0,
			                "fx": 234.21713414452663,
			                "fy": 203.9996885862305
			            },
			            "index": 0
			            },
			            {
			        ...
			        node.append("rect")
			            .attr("width", d => Math.max(d.title.length * 8, 100))
			            .attr("height", 200)
			            .attr("x", d => -Math.max(d.title.length * 8, 100) / 2)
			            .attr("y", -20)
			            .style("stroke", d => getNodeColor(d.status));
			        
			        // Add status indicator
			        node.append("circle")
			            .attr("r", 5)
			            .attr("cx", d => Math.max(d.title.length * 8, 100) / 2 - 10)
			            .attr("cy", -10)
			        ...
			        function ticked() {
			            link.attr("d", d => {
			                const sourceNode = graphData.nodes.find(node => node.id === d.source.id || node.id === d.source);
			                const targetNode = graphData.nodes.find(node => node.id === d.target.id || node.id === d.target);
			        ...
			        function showPopover(event, d) {
			            ...
			            if (d.parentId) {
			                const parentNode = graphData.nodes.find(node => node.id === d.parentId);
			                if (parentNode) parentTitle = parentNode.title;
			            }
			            
			            // Update popover content
			            d3.select("#popover-title").text(d.title);
			            d3.select("#popover-id").text(d.id);
			            d3.select("#popover-parent").text(parentTitle);
			            d3.select("#popover-children").text(d.children.length);
			            d3.select("#popover-created").text(d.created);
			            d3.select("#popover-status").text(d.status);
			        ...
			        function closePopover() {
			            d3.select("#popover").classed("active", false);
			        ...
			        d3.select("#add-node").on("click", () => {
			            ...
			            const parentIndex = Math.floor(Math.random() * graphData.nodes.length);
			            const parentNode = graphData.nodes[parentIndex];
			            const newId = (Math.max(...graphData.nodes.map(n => parseInt(n.id))) + 1).toString();
