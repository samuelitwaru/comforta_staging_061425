function CKEditorControl() {
  ...
  this.Toolbar;
  this.MenuMap = [];
  this.NotSupportedDevice =
    (gx.util.browser.isIPhone() || gx.util.browser.isIPad()) &&
    navigator.userAgent.match(/OS [1-4]{1}(_\d)+ like Mac OS X/i);
  ...
  this.show = function (force) {
    ...
    if (!this.IsPostBack) {
      if (!gx.lang.gxBoolean(this.Enabled)) this.SetData(this.data);
      else {
        if (this.NotSupportedDevice) {
          var buffer =
            '<textarea id="' +
            this.ContainerName +
            '_textArea" style="width:' +
            this.Width +
            ";height:" +
            this.Height +
            '">' +
  ...
  this.Initialize = function () {
    ...
    this.BasePath = gx.util.resourceUrl(
      gx.basePath + gx.staticDirectory + "CKEditor/",
      true
    );
    if (
      !gx.lang.emptyObject(this.CustomToolbar) &&
      !gx.lang.emptyObject(this.Toolbar) &&
      this.Toolbar == "Custom"
    )
  ...
  this.CreateCkEditor = function () {
    ...
    if (this.MenuGroups) instanceConfig.menu_groups = this.MenuGroups;
    ...
    if (!this.MenuGroups) this.MenuGroups = this.editor.config.menu_groups;
  ...
  this.ButtonClickHandler = function (id) {
    ...
    if (itemProperties.ObjectInterface == 1) this.OpenPrompt(itemProperties);
    if (itemProperties.ObjectInterface == 2) this.GXEvent(itemProperties);
  };
  ...
  this.OpenPrompt = function (itemProperties) {
    this.handle = gx.popup.openUrl(gx.http.formatLink(itemProperties.Link), [
      "OutPutParm1",
    ]);
  };
  ...
  this.CallHttpProcedure = function (itemProperties) {
    ...
    if (response != "") this.SendContentToEditor(response);
  };
    ...
    if (this.ToolbarReference) {
      ...
      for (var i = 0; i < this.Menu.length; i++) {
        var btn = {
          ...
          icon: gx.util.resourceUrl(
            (gx.gen.ruby ? gx.basePath : "") +
              gx.staticDirectory.substring(1, gx.staticDirectory.length) +
              this.Menu[i].Icon,
            true
          ),
          click: this.CreateDelegate(this, this.ButtonClickHandler, [
            this.Menu[i].Id,
          ]),
        };
  ...
  this.GetMenuObject = function (menuItem) {
    ...
    obj[menuItem.Id] = {
      ...
      onClick: this.CreateDelegate(this, this.ButtonClickHandler, [
        menuItem.Id,
      ]),
      group: "insertCode",
    };
  ...
  this.SetMode = function (mode, text) {
    if (!this.NotSupportedDevice) {
      ...
      if (mode == 1 && this.editor == null) {
        var data =
          text != undefined ? text : this.getContainerControl().innerHTML;
        this.setHtml("");
        this.CreateCkEditor();
  ...
  this.SetMenu = function (menu) {
    if (!this.NotSupportedDevice) {
      this.Menu = menu;
      if (!this.ToolbarReference) {
        this.ToolbarReference =
          CKEDITOR.config["toolbar_" + this.ToolbarType] || [];
  ...
  this.SetInitialData = function (data) {
    ...
    else document.getElementById(this.ContainerName).innerHTML = this.data;
  };
  ...
  this.SetData = function (data) {
    if (!gx.lang.gxBoolean(this.Enabled)) {
      ...
      if (this.NotSupportedDevice)
        gx.dom.byId(this.ContainerName + "_textArea").value = this.data;
      else this.editor.setData(this.data);
    }
  };
