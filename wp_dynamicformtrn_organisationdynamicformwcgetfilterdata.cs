...
namespace GeneXus.Programs {
   public class wp_dynamicformtrn_organisationdynamicformwcgetfilterdata : GXProcedure
   {
      ...
      public wp_dynamicformtrn_organisationdynamicformwcgetfilterdata( )
      {
      ...
      public wp_dynamicformtrn_organisationdynamicformwcgetfilterdata( IGxContext context )
      {
      ...
      protected void S111( )
      {
         ...
         returnInSub = false;
         if ( StringUtil.StrCmp(AV32Session.Get("WP_DynamicFormTrn_OrganisationDynamicFormWCGridState"), "") == 0 )
         {
            AV34GridState.FromXml(new WorkWithPlus.workwithplus_web.loadgridstate(context).executeUdp(  "WP_DynamicFormTrn_OrganisationDynamicFormWCGridState"), null, "", "");
         }
         else
         {
            AV34GridState.FromXml(AV32Session.Get("WP_DynamicFormTrn_OrganisationDynamicFormWCGridState"), null, "", "");
         }
         AV51GXV1 = 1;
      ...
      protected void S121( )
      {
         ...
         /* Using cursor P00DG2 */
         pr_default.execute(0, new Object[] {AV46WWPFormType, AV48OrganisationId, lV11TFWWPFormTitle, AV12TFWWPFormTitle_Sel, AV15TFWWPFormDate, AV16TFWWPFormDate_To, AV17TFWWPFormVersionNumber, AV18TFWWPFormVersionNumber_To});
         while ( (pr_default.getStatus(0) != 101) )
         {
            BRKDG2 = false;
            A240WWPFormType = P00DG2_A240WWPFormType[0];
            A11OrganisationId = P00DG2_A11OrganisationId[0];
            A209WWPFormTitle = P00DG2_A209WWPFormTitle[0];
            A231WWPFormDate = P00DG2_A231WWPFormDate[0];
            A207WWPFormVersionNumber = P00DG2_A207WWPFormVersionNumber[0];
            A206WWPFormId = P00DG2_A206WWPFormId[0];
            A509OrganisationDynamicFormId = P00DG2_A509OrganisationDynamicFormId[0];
            A240WWPFormType = P00DG2_A240WWPFormType[0];
            A209WWPFormTitle = P00DG2_A209WWPFormTitle[0];
            A231WWPFormDate = P00DG2_A231WWPFormDate[0];
            ...
            if ( String.IsNullOrEmpty(StringUtil.RTrim( AV43FilterFullText)) || ( ( StringUtil.Like( StringUtil.Lower( A209WWPFormTitle) , StringUtil.PadR( "%" + StringUtil.Lower( AV43FilterFullText) , 255 , "%"),  ' ' ) ) || ( StringUtil.Like( StringUtil.Str( (decimal)(A207WWPFormVersionNumber), 4, 0) , StringUtil.PadR( "%" + AV43FilterFullText , 254 , "%"),  ' ' ) ) || ( StringUtil.Like( StringUtil.Str( (decimal)(A219WWPFormLatestVersionNumber), 4, 0) , StringUtil.PadR( "%" + AV43FilterFullText , 254 , "%"),  ' ' ) ) ) )
            {
               if ( (0==AV19TFWWPFormLatestVersionNumber) || ( ( A219WWPFormLatestVersionNumber >= AV19TFWWPFormLatestVersionNumber ) ) )
               {
                  if ( (0==AV20TFWWPFormLatestVersionNumber_To) || ( ( A219WWPFormLatestVersionNumber <= AV20TFWWPFormLatestVersionNumber_To ) ) )
                  {
                     ...
                     AV31count = 0;
                     while ( (pr_default.getStatus(0) != 101) && ( P00DG2_A240WWPFormType[0] == A240WWPFormType ) && ( StringUtil.StrCmp(P00DG2_A209WWPFormTitle[0], A209WWPFormTitle) == 0 ) )
                     {
                        BRKDG2 = false;
                        A11OrganisationId = P00DG2_A11OrganisationId[0];
                        A207WWPFormVersionNumber = P00DG2_A207WWPFormVersionNumber[0];
      ...
      public override void initialize( )
      {
         ...
         AV48OrganisationId = Guid.Empty;
         P00DG2_A240WWPFormType = new short[1] ;
         P00DG2_A11OrganisationId = new Guid[] {Guid.Empty} ;
         P00DG2_A209WWPFormTitle = new string[] {""} ;
         P00DG2_A231WWPFormDate = new DateTime[] {DateTime.MinValue} ;
         P00DG2_A207WWPFormVersionNumber = new short[1] ;
         P00DG2_A206WWPFormId = new short[1] ;
         P00DG2_A509OrganisationDynamicFormId = new Guid[] {Guid.Empty} ;
         A509OrganisationDynamicFormId = Guid.Empty;
         AV26Option = "";
         pr_default = new DataStoreProvider(context, new GeneXus.Programs.wp_dynamicformtrn_organisationdynamicformwcgetfilterdata__default(),
            new Object[][] {
                new Object[] {
               P00DG2_A240WWPFormType, P00DG2_A11OrganisationId, P00DG2_A209WWPFormTitle, P00DG2_A231WWPFormDate, P00DG2_A207WWPFormVersionNumber, P00DG2_A206WWPFormId, P00DG2_A509OrganisationDynamicFormId
               }
            }
         );
         /* GeneXus formulas. */
      ...
      private Guid AV48OrganisationId ;
      ...
      private short[] P00DG2_A240WWPFormType ;
      private Guid[] P00DG2_A11OrganisationId ;
      private string[] P00DG2_A209WWPFormTitle ;
      private DateTime[] P00DG2_A231WWPFormDate ;
      private short[] P00DG2_A207WWPFormVersionNumber ;
      private short[] P00DG2_A206WWPFormId ;
      private Guid[] P00DG2_A509OrganisationDynamicFormId ;
      private string aP3_OptionsJson ;
      private string aP4_OptionsDescJson ;
      private string aP5_OptionIndexesJson ;
   }

   public class wp_dynamicformtrn_organisationdynamicformwcgetfilterdata__default : DataStoreHelperBase, IDataStoreHelper
   {
      protected Object[] conditional_P00DG2( IGxContext context ,
                                             ...
                                             short A240WWPFormType )
      {
         ...
         scmdbuf = "SELECT T2.WWPFormType, T1.OrganisationId, T2.WWPFormTitle, T2.WWPFormDate, T1.WWPFormVersionNumber, T1.WWPFormId, T1.OrganisationDynamicFormId FROM (Trn_OrganisationDynamicForm T1 INNER JOIN WWP_Form T2 ON T2.WWPFormId = T1.WWPFormId AND T2.WWPFormVersionNumber = T1.WWPFormVersionNumber)";
         AddWhere(sWhereString, "(T2.WWPFormType = :AV46WWPFormType)");
         AddWhere(sWhereString, "(T1.OrganisationId = :AV48OrganisationId)");
    ...
    private void cursorDefinitions( )
    {
       if ( def == null )
       {
          ...
          new ParDef("AV48OrganisationId",GXType.UniqueIdentifier,36,0) ,
          ...
          def= new CursorDef[] {
              new CursorDef("P00DG2", "scmdbuf",false, GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK, false, this,prmP00DG2,100, GxCacheFrequency.OFF ,true,false )
          };
       }
    }
