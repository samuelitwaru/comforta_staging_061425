...
export class InfoContentMapper {
  ...
  public pasteAllInfoSectionTypes(
    sections: any[],
  ...
  public handleDragAndDropToNewTileArea(
    draggedTileId: string,
    draggedTileParentId: string,
    ...
    if (draggedTileParent && draggedTile) {
      ...
      if (
        draggedTileParent.Columns.length === 1 &&
        draggedTileParent.Columns[0].Tiles?.length > 1
      ) {
        ...
        tiles.slice(1).forEach((tile: any, index: number) => {
          ...
          const draggedTileParentIndex =
            data.PageInfoStructure.InfoContent.findIndex(
              (section: any) => section.InfoId === draggedTileParentId
            );
          ...
          if (draggedTileParentIndex === -1) {
            ...
            data.PageInfoStructure.InfoContent.splice(
              draggedTileParentIndex + 1 + index,
              0,
              content
            );
          }
  ...
  public handleDragAndDropToExistingTileArea(
    draggedTileId: string,
  ...
  public handleDragAndDropWithinExistingTileColumn(
    draggedTileId: string,
    draggedFromParentId: string,
    draggedToParentId: string,
    tileSectionId: string,
    tileDestinationIndex: string
  ): any {
    const storageKey = `data-${this.pageId}`;
    const data: any = JSON.parse(localStorage.getItem(storageKey) || "{}");
  ...
  public handleDragAndDropTileToExistingTileColumn(
    draggedTileId: string,
    draggedFromParentId: string,
    draggedToColumnId: string,
    ...
    if (targetSection && draggedTile) {
      ...
      targetSection.Columns = targetSection.Columns.map((col: any) => {
        if (col.ColId === draggedToColumnId) {
          ...
          const draggedTileContent = draggedTile.Tiles
            ? draggedTile.Tiles[0]
            : draggedTile;
          // Insert the dragged tile at the specified index
          col.Tiles.splice(tileDestinationIndex, 0, {
  ...
  public handleDragAndDropTileWrapperToNewTileColumn(
    draggedTileId: string,
    draggedFromColumnId: string,
    draggedFromTileSectionId: string,
    beforeSectionId?: string
  ): any {
    const storageKey = `data-${this.pageId}`;
    const data: any = JSON.parse(localStorage.getItem(storageKey) || "{}");
    ...
    if (draggedTileParentColumn && draggedTile) {
      ...
      if (draggedTileParentColumn.Tiles.length === 0) {
        draggedTileParentSection.Columns =
          draggedTileParentSection.Columns.filter(
            (col: any) => col.ColId !== draggedFromColumnId
          );
      }
  ...
  checkAndRemoveEmptyTileSections(data: any): any {
    ...
    data.PageInfoStructure.InfoContent =
      data.PageInfoStructure.InfoContent.filter((section: any) => {
        ...
        if (section.InfoType === "TileGrid") {
          if (section.Columns && section.Columns.length > 0) {
            return section.Columns.some(
              (col: any) => col.Tiles && col.Tiles.length > 0
            );
          }
  ...
  public cutInfoSectionsFromPage(
    sectionIdsToRemove: any[],
    cutPageId: string
  ): any {
