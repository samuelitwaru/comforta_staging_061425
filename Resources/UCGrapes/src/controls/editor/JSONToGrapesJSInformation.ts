...
export class JSONToGrapesJSInformation {
  ...
  private generateCta(infoContent: InfoType) {
    const cta = infoContent.CtaAttributes;
    if (cta?.CtaButtonType === "Image") {
      return `<div id="${infoContent.InfoId}" data-gjs-type="info-cta-section"
                ${ctaTileDEfaultAttributes} 
      ...
      return `
          <div id="${infoContent.InfoId}" data-gjs-type="info-cta-section"
            ${ctaTileDEfaultAttributes} 
              button-type="${cta.CtaType}"
              class="img-button-container">
                <div ${DefaultAttributes} class="img-button cta-styled-btn"
      ...
      switch (cta.CtaType) {
        ...
        <svg ${DefaultAttributes} id="ixdtl" data-gjs-type="svg" xmlns="http://www.w3.org/2000/svg" width="32" height="32"
            ...
            <path ${DefaultAttributes} id="call" data-gjs-type="svg-in"
                ...
                transform="translate(-4 -3)" fill="${cta.CtaColor ? cta.CtaColor : "#ffffff"
            }"></path>
        </svg>
        `;
          break;
        ...
        <svg ${DefaultAttributes} id="inavf" data-gjs-type="svg" xmlns="http://www.w3.org/2000/svg" width="32"
            ...
            <path ${DefaultAttributes} id="Path_1218" data-gjs-type="svg-in" data-name="Path 1218"
                ...
                transform="translate(-2 -4)" fill="${cta.CtaColor ? cta.CtaColor : "#ffffff"
            }"></path>
        </svg>
        `;
          break;
        ...
        <svg ${DefaultAttributes} id="igqdh" data-gjs-type="svg" xmlns="http://www.w3.org/2000/svg" width="26" height="30"
              ...
              <path ${DefaultAttributes} id="Path_1209" data-gjs-type="svg-in" data-name="Path 1209"
                  ...
                  transform="translate(-8 -4)" fill="${cta.CtaColor ? cta.CtaColor : "#ffffff"
            }"></path>
          </svg>
        `;
          break;
  ...
  private generateInfoContent(content: InfoType): any {
    if (content.InfoType === "Image" && content.InfoValue !== "") {
      return this.contentInfoImage(content);
    } else if (content.InfoType === "Images" && content.Images) {
      return this.contentInfoImages(content);
    } else if (content.InfoType === "Description" && content.InfoValue !== "") {
      return this.contentInfoDescription(content);
      ...
      const getTileHtml = new JSONToGrapesJSMenu(this.data).generateInfoRow(
        content
      );
      return getTileHtml;
  ...
  public generateHTML(): any {
    ...
    if (Array.isArray(infoContent) && infoContent.length > 0) {
      ...
      contentHtml = new AddInfoSectionButton(
        false,
        true,
        this.isNewPage || this.data.PageName === "Untitled"
      ).getHTML();
    }

    const htmlData = `
        <div ${DefaultAttributes} id="frame-container" data-gjs-type="template-wrapper" class="${containerClass}">
  ...
  contentInfoImage(content: InfoType) {
    return `
    <div ${contentDefaultAttributes} class="info-image-section" id="${content?.InfoId ? content?.InfoId : randomIdGenerator(15)
      }" data-gjs-type="info-image-section">
    <button ${DefaultAttributes} class="tb-edit-image-icon"
        ${this.data?.PageType === "Location" ||
        this.data?.PageType === "Reception"
        ? `style="right: -20px"`
        : ``
      }>
  ...
  contentInfoImages(content: InfoType) {
    const values: Image[] = content.Images || [];
    ...
    <div ${contentDefaultAttributes} class="info-image-section" id="${content?.InfoId ? content?.InfoId : randomIdGenerator(15)
      ...
      <button ${DefaultAttributes} class="tb-delete-image-icon">
          <svg ${DefaultAttributes} fill="#5068a8" width="14px" height="14px"
              ...
              <title ${DefaultAttributes} data-gjs-type="svg-in">delete</title>
              <path ${DefaultAttributes} fill="#5068a8"
  ...
  private singleImage(image: any, index: number, length: number): string {
    const displayStyle = index === 1 ? "display: block;" : "display: none;";
    return `
    <div ${DefaultAttributes} data-gjs-type="default" class="mySlides fade" style="${displayStyle}">
  ...
  contentInfoDescription(content: InfoType) {
    return `<div
