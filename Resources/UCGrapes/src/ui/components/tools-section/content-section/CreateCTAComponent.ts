...
export class CreateCTAComponent {
  ...
  renderModalFooter() {
    ...
    cancelBtn.classList.add("tb-btn", "tb-btn-outline");
  ...
  validateCta(data: any) {
    ...
    if (!data.CtaLabel || !data.CtaType || !data.CtaValue) {
      return {
        isValid: false,
        message: `Type, Label, and ${data.CtaType} are required.`,
      };
    }
    ...
    switch (data.CtaType) {
      case "Phone":
        if (!phoneRegex.test(data.CtaValue)) {
          return {
            isValid: false,
            message: "CtaValue must be a 10-digit number for Phone type.",
          };
        }
        break;
      case "Url":
      case "Form":
        if (!urlRegex.test(data.CtaValue)) {
          return {
            isValid: false,
            message: "CtaValue must be a valid URL for Url or Form type.",
          };
        }
        break;
      case "Email":
        if (!emailRegex.test(data.CtaValue)) {
          return {
            isValid: false,
            message: "CtaValue must be a valid email address for Email type.",
          };
        }
        break;
      default:
