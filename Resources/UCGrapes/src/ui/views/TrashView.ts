...
export class TrashView {
  ...
  private async createPageTabContent(): Promise<HTMLDivElement> {
    ...
    if (
      ...
      Array.isArray(trashItems.TrashItems)
    ) {
      const pages = trashItems.TrashItems.filter(
        (item) => item.Type === "Page"
      );
  ...
  private async createVersionTabContent(): Promise<HTMLDivElement> {
    ...
    if (
      ...
      const versions = trashItems.TrashItems.filter(
  ...
  private createListItem(item: TrashItem): HTMLDivElement {
    ...
    let itemDeletedDateTime: string = formatDistanceToNow(new Date(), {
      addSuffix: true,
    });

    if (item.Type === "Page") {
      itemTitle = item.Page.PageName;
      itemDeletedDateTime = formatDistanceToNow(new Date(item.DeletedAt), {
        addSuffix: true,
      });
  ...
  private setupTabSwitching(container: HTMLDivElement): void {
    ...
    tabs.forEach((tab) => {
      tab.addEventListener("click", () => {
        container
          .querySelectorAll(".trash-tab")
          .forEach((t) => t.classList.remove("active"));
        container
          .querySelectorAll(".trash-tab-content")
  ...
  private setupDeleteForeverButton(
    ...
    deleteForeverBtn.addEventListener("click", () => {
      ...
      if (checkedItems.length > 0) {
        if (
          confirm(
            `Permanently delete ${checkedItems.length} selected item(s)? This action cannot be undone.`
          )
        ) {
  ...
  private removeSelectedItems(
    checkedItems: NodeListOf<Element>,
    mode: string
  ): void {
  ...
  private setupEmptyTrashButton(
    container: HTMLDivElement,
    emptyTrashBtn: Element
  ): void {
      ...
      if (activeTab) {
        ...
        if (tabId) {
          ...
          if (
            ...
            if (activeContent) {
              const tabItems =
                activeContent.querySelectorAll(".trash-checkbox");
              if (tabItems) {
